...
<body>
	<div style="overflow-x: hidden;">
		...
		<div class="row align-items-center"
			...
			<jsp:include page="../resources/jsp/sidebar.jsp">
				<jsp:param name="icon" value="carlist" />
			</jsp:include>
			<div class="col-10"
							...
							<div class="col-auto" style="margin-right: 1%; width: 30%;">
								...
								<div class="input-group mb-2">
									...
									<input id="search" name="search" class="form-control"
										type="text" placeholder="Car Search" maxlength="50"
										value="${search}">
								</div>
							</div>
							<div class="col-auto" style="margin-right: 1%; width: 20%;">
								...
								<div class="input-group mb-2">
									...
									<select id="by" name="by" style="width: 55%; margin: 0;"
										class="box">
										<option value="carType" ${by=='carType'?'selected':''}>Car
											Type</option>
										<option value="company" ${by=='company'?'selected':''}>Company</option>
									</select>
								</div>
							</div>
				...
				<div style="margin: 1% auto 2%; clear: both;">
					<table style="width: 100%;">
						...
						<c:forEach items="${listcar}" var="r">
							<tr>
								<td>${r.getLicensePlate()}</td>
								<td>${r.getCarType()}</td>
								<td>${r.getCarColor()}</td>
								<td>${r.getCompany()}</td>
								<td>${r.getParkId()}</td>
								<td><a href="LoadCar?licensePlate=${r.getLicensePlate()}"
									style="color: #5BA4D6;"> <i class="fas fa-edit"></i>Edit
								</a></td>
								<td><a href="deletecar?licensePlate=${r.getLicensePlate()}"
									class="delete" style="color: #5BA4D6;"> <i
	...
	$(".btn-last").on("click", (e) => {
		...
		$.ajax(
			{
				url: 'http://localhost:8080/CarPark/ad/listcar',
				type: 'POST',
				data: {
